...
func TestLoginHandler(t *testing.T) {
	...
	auth := &cookie.CookieAuth{
		SessionManager: cookie.NewSessionManager(repo),
		UserRepo:       repo,
	}
	...
	for _, c := range resp.Cookies() {
		if c.Name == cookie.SessionTokenCookieKey {
			found = true
			break
		}
	}
	...
	if !found {
		t.Errorf("cookie '%s' not found in response", cookie.SessionTokenCookieKey)
	}
}
...
func TestLoginHandler_InvalidBody(t *testing.T) {
	...
	for _, c := range resp.Cookies() {
		if c.Name == cookie.SessionTokenCookieKey {
			t.Errorf("unexpected cookie '%s' found in response", cookie.SessionTokenCookieKey)
			break
		}
	}
}
...
func TestLoginHandler_InvalidPassword(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader(`{"username": "username", "password": "pass"}`))
	w := httptest.NewRecorder()
...
func TestProtectedHandler(t *testing.T) {
	...
	sessionManager := &cookie.CookieSessionManager{
	...
	req.AddCookie(&http.Cookie{
		Name:  cookie.SessionTokenCookieKey,
		Value: token,
	})
	w := httptest.NewRecorder()
...
func TestProtectedHandler_MissingCookie(t *testing.T) {
	repo := &mockUserRepo{
		users: map[string]storage.User{
			"username": &mockUser{
				username: "username",
